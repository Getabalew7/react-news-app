...
export const Login = () => {
  ...
  //lets handle user data change
  const handleChangeData = (e) => {
    setUserData({ ...userData, [e.target.name]: e.target.value });
  };
  //lets handle user login
  const handleLogin = (e) => {
    e.preventDefault();
    //validate inputs
          ...
          axios.get('http://127.0.0.1:8000/sanctum/csrf-cookie').then(res=>{
            ...
            .then((response) => {
              if (response.data.status === 200) {
                ...
                swal("warning", response.data.validator_errors, "warning");
              }
            });
          });
    } else {
      swal("warning", "One or more inputs are Empty", "warning");
    }
  };
  return (
    <div className="py-0">
